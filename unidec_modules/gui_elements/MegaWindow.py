...
class MegaWindow(mainwindow_base.MainwindowBase):
    def __init__(self, parent, title, config=None, iconfile="logo.ico", *args, **kwargs):
        ...
        #########
        #  LJC EDIT add option for megaunidec - 
        # auto peak selection and transfer to metaunidec
        tsizer3 = wx.BoxSizer(wx.HORIZONTAL)
        self.megabutton = wx.Button(self.panel, label = 'Auto TIC peak selection')
        self.Bind(wx.EVT_BUTTON, self.pres.on_mega_run, self.megabutton)
        tsizer3.Add(self.megabutton)
